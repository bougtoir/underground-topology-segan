"""Phase 12 (v2): S3 = S2 topology + rooftop-PV distributed generation.
...
for name in ["sano", "kudamatsu", "yasu"]:
    ...
    pv_node = (pd.Series(b["footprint_m2"].values * ROOF * PV_KW_M2,
               ...
               .groupby(level=0).sum())
    grp = pd.read_csv(f"{ROOT}/data/processed/{name}/clusters_v2.csv")
    ...
    s3.to_csv(f"{ROOT}/data/processed/{name}/s3_results_v2.csv", index=False)
    ...
    print(rows[-1])
pd.DataFrame(rows).to_csv(f"{ROOT}/analysis/dg_results_v2.csv", index=False)
print("done")
