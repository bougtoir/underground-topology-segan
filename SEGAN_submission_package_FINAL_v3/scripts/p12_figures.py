"""Phase 12: manuscript figures (PNG, 300 dpi, separate files per journal
...
dg = pd.read_csv(f"{ROOT}/analysis/dg_results.csv").set_index("city")
tor = pd.read_csv(f"{ROOT}/analysis/sensitivity_tornado.csv")
...
fig.suptitle("Study zones: OSM road graph (grey) and snapped building demand nodes (blue)",
             fontsize=9, y=1.02)
fig.tight_layout()
fig.savefig(f"{FIG}/fig1_study_zones.png", bbox_inches="tight"); plt.close(fig)
...
import glob
f1 = sorted(glob.glob(f"{ROOT}/data/processed/{name}/recon/c000_r0*_A_steiner.csv"))[0]
...
s2 = nx.read_edgelist(f"{ROOT}/data/processed/{name}/s2_cluster0.edgelist",
                      data=False)
for u, v in s2.edges():
    ax = axes[1]
    ax.plot([pos[u][0], pos[v][0]], [pos[u][1], pos[v][1]],
...
fig.suptitle("Example feeder cluster (Sano c000): S1 vs S2 layout", fontsize=9)
fig.tight_layout()
fig.savefig(f"{FIG}/fig2_topology_example.png", bbox_inches="tight"); plt.close(fig)
...
ax.set_ylabel("Life-cycle topology profit (% of S1 NPC)")
ax.legend(fontsize=7)
fig.tight_layout()
fig.savefig(f"{FIG}/fig3_ltp.png", bbox_inches="tight"); plt.close(fig)
...
ax.set_xlabel("LTP (%) under one-at-a-time parameter swing (Sano)")
fig.tight_layout()
fig.savefig(f"{FIG}/fig4_tornado.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{FIG}/fig5_dg.png", bbox_inches="tight"); plt.close(fig)

# Tables as CSVs ----------------------------------------------------------
tab = scen[["clusters", "total_demand_kw", "s1_npc", "s2_npc", "delta_npc",
            "ltp_pct", "p_ltp_pos", "mean_tld"]].copy()
tab["total_demand_kw"] /= 1000
for c in ["s1_npc", "s2_npc", "delta_npc"]:
    tab[c] /= 1e9
tab.columns = ["clusters", "demand_MW", "S1_NPC_bn", "S2_NPC_bn",
               "Delta_NPC_bn", "LTP_pct", "P_LTP_pos", "mean_TLD"]
tab.to_csv(f"{ROOT}/tables/table2_results.csv")
