...
for name in ["sano", "kudamatsu", "yasu"]:
    ...
    for c in range(k):
        ...
        if not all(t in H for t in terms):
            miss = [t for t in terms if t not in H]
            H = nx.compose(H, G.subgraph(miss))
        H = H.subgraph(max(nx.connected_components(H), key=len)).copy() if len(list(nx.connected_components(H))) > 1 else H
        ...
        for r in range(2 * M):
            ...
            ed = pd.DataFrame(list(net.edges), columns=["u", "v"])
    ...
    pd.DataFrame([{"city": kk[0], "cluster": kk[1], "u": kk[2], "v": kk[3], "freq": vv/(2*M)}
                  for kk, vv in edge_freq.items()]
                 ).to_csv(f"{outdir}/edge_presence_freq.csv", index=False)
    n.to_csv(f"{ROOT}/data/processed/{name}/nodes_with_cluster.csv", index=False)
    ...
    print(name, "clusters", n_clusters, "replicates written",
