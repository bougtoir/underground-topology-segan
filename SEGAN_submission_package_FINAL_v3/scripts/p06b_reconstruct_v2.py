...
Outputs go to recon_"+TAG/ and clusters_"+TAG+".csv; v1 outputs untouched."""
import os, math
import numpy as np
import pandas as pd
...
M = int(os.environ.get("NREP", CFG["reconstruction"]["n_replicates"])) // 2
SEED = CFG["project"]["seed"]
TAG = os.environ.get("TAG", "v2")
CLUSTER_KW = float(os.environ.get("CLUSTER_KW", SPEC["demand_model"]["cluster_target_kw"]))
LV_MAX = float(os.environ.get("LV_MAX", SPEC["demand_model"]["lv_max_node_kw"]))
BUFFER_M = 150.0
...
for name in ["sano", "kudamatsu", "yasu"]:
    ...
    outdir = f"{ROOT}/data/processed/{name}/recon_"+TAG
    ...
    pd.DataFrame({"node": list(cluster_of.keys()), "cluster": list(cluster_of.values())}
                 ).to_csv(f"{ROOT}/data/processed/{name}/clusters_"+TAG+".csv", index=False)
    print(name, "clusters done; LV nodes", len(dnodes), "MV excluded", len(mv_nodes))
pd.DataFrame(summary).to_csv(f"{ROOT}/analysis/reconstruction_summary_"+TAG+".csv", index=False)
pd.DataFrame(mv_rows).to_csv(f"{ROOT}/analysis/mv_served_nodes_"+TAG+".csv", index=False)
print("done")
