...
Outputs: data/processed/<city>/cluster_results_v2.csv,
         analysis/scenario_results_v2.csv, analysis/ablation_results.csv
...
for name in ["sano", "kudamatsu", "yasu"]:
    ...
    cl = pd.read_csv(f"{ROOT}/data/processed/{name}/clusters_v2.csv")
    cl["node"] = cl["node"].astype(str)
    recon_sum = pd.read_csv(f"{ROOT}/analysis/reconstruction_summary_v2.csv")
    ...
    for c, grp in cl.groupby("cluster"):
        ...
        s1, s1cond, s1_tree = [], [], []
        for f in sorted(glob.glob(f"{outdir}/recon_v2/c{int(c):03d}_r*_*csv")):
            ed = pd.read_csv(f)
            T = nx.Graph()
            ok = True
        ...
        res_c.append(dict(
            ...
            s1_npc_mean=np.mean([m["npc"] for m in s1]) if s1 else np.nan,
            s1_npc_feas_mean=np.mean([m["npc"] for m in s1_ok]) if s1_ok else np.nan,
            s1_capex_mean=np.mean([m["capex"] for m in s1]) if s1 else np.nan,
            s1_length_mean=np.mean([m["length_m"] for m in s1]) if s1 else np.nan,
            s1_plkw_mean=np.mean([m["pl_kw"] for m in s1]) if s1 else np.nan,
            s1_vdrop_max=max(m["vdrop_pct"] for m in s1) if s1 else np.nan,
            ...
            tld=tld))
        if not os.path.exists(f"{outdir}/s2_cluster0_v2.edgelist") and best_toc is not None:
            nx.write_edgelist(cand[best_toc],
                              f"{outdir}/s2_cluster0_v2.edgelist", data=["length"])
    rc = pd.DataFrame(res_c)
    rc.to_csv(f"{outdir}/cluster_results_v2.csv", index=False)
    ...
    print(rows[-1])
pd.DataFrame(rows).to_csv(f"{ROOT}/analysis/scenario_results_v2.csv", index=False)
print("done")
