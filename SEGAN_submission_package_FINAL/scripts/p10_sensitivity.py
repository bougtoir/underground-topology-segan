"""Phases 9-10: Monte-Carlo driver sensitivity on stored S1/S2 components.
...
of scope). Outputs analysis/sensitivity_mc.csv + analysis/sensitivity_tornado.csv."""
import numpy as np
import pandas as pd
import yaml, os
...
for name in ["sano", "kudamatsu", "yasu"]:
    ...
    c1, p1 = rc["s1_capex_mean"].values, rc["s1_plkw_mean"].values
    c2, p2 = rc["s2_capex"].values, rc["s2_plkw"].values
...
pd.DataFrame(mc_rows).to_csv(f"{ROOT}/analysis/sensitivity_mc.csv", index=False)
pd.DataFrame(tor_rows).to_csv(f"{ROOT}/analysis/sensitivity_tornado.csv", index=False)
for r in mc_rows:
    print(r)
print("done")
