"""Phase R13 (v3): manuscript figures (PNG, 300 dpi, separate files per journal
...
fig.suptitle("Study zones: OSM road graph (grey) and snapped building demand nodes (blue)",
             fontsize=9, y=1.02)
fig.tight_layout()
fig.savefig(f"{FIG}/v3_fig1_study_zones.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{FIG}/v3_fig2_topology_example.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{FIG}/v3_fig3_delta.png", bbox_inches="tight"); plt.close(fig)

# Fig 4: engineering-assumption sensitivity (all cities, delta_npc %)
...
for ax, name in zip(axes, CITIES):
    ...
    ax.set_title(f"{CNAMES[name]} (base {base:+.2f}%)", fontsize=8)
    ax.set_yticks(ys); ax.set_yticklabels(labels, fontsize=7)
    ax.set_xlabel("ΔNPC %", fontsize=7)
fig.suptitle("ΔNPC under engineering-assumption swings (1% equivalence margin shaded region not shown)", fontsize=8)
fig.tight_layout()
fig.savefig(f"{FIG}/v3_fig4_tornado.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{FIG}/v3_fig5_dg.png", bbox_inches="tight"); plt.close(fig)

# Tables as CSVs ----------------------------------------------------------
tab = scen[["clusters", "clusters_feasible", "lv_demand_kw", "s1_npc", "s2_npc", "delta_npc",
            "ltp_pct", "vof", "vof_pct", "p_vof_pos", "p_ltp_pos", "mean_tld"]].copy()
...
tab.columns = ["clusters", "clusters_feasible", "demand_MW", "S1_NPC_bn", "S2_NPC_bn",
               "Delta_NPC_bn", "Delta_NPC_pct", "VoF_bn", "VoF_pct", "P_VoF_pos",
               "P_LTP_pos", "mean_TDD"]
tab.to_csv(f"{ROOT}/tables/table2_results_v3.csv")
