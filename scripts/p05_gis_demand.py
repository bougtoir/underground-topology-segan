...
demand density, clustered onto road nodes without empirical load calibration."""
import os, json
...
for name, meta in CITIES.items():
    ...
    if meta["anchor_hint"]:
        ...
        bb_projected = bb.to_crs(bb.estimate_utm_crs())
        c = gpd.GeoSeries(
            bb_projected.geometry.centroid,
            crs=bb_projected.crs,
        ).to_crs(4326)
    ...
    bu = b.to_crs(utm)
    bu["footprint_m2"] = bu.geometry.area
    bu["geometry"] = bu.geometry.centroid
